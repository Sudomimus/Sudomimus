...
public static class ClientJwtSigner
{
    ...
    public static string Sign(
        ConnectClientAuthWithKey config,
        string rawBody,
        DateTimeOffset now)
    {
        var lifetime = config.LifetimeSeconds ?? ConnectConstants.ClientJwtDefaultLifetimeSeconds;
        if (lifetime <= 0 || lifetime > ConnectConstants.ClientJwtMaxLifetimeSeconds)
        {
            throw new ConnectConfigException(
                $"ClientAuth.LifetimeSeconds must be in (0, {ConnectConstants.ClientJwtMaxLifetimeSeconds}]; got {lifetime}.");
        }
        ...
        var claims = new Dictionary<string, object>
        {
            ...
            ["aud"] = ConnectConstants.ClientJwtAudience,
            ["iat"] = iat,
            ["exp"] = exp,
            ["jti"] = jti,
