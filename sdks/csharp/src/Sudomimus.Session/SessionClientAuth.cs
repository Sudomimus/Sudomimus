...
/// </summary>
public delegate Task<string> SessionClientAuthSigner(string rawBody, CancellationToken cancellationToken);

/// <summary>
/// Configuration for the client-auth JWT required by <c>/revoke-all</c>. Use a derived type:
/// <see cref="SessionClientAuthWithKey"/> to let the SDK sign with a
/// PEM-encoded RSA private key, or <see cref="SessionClientAuthWithSigner"/>
/// to delegate signing to an external system (HSM, KMS, etc.).
/// </summary>
public abstract record SessionClientAuth
{
...
/// </summary>
public sealed record SessionClientAuthWithKey : SessionClientAuth
{
    ...
    /// <see cref="SessionConstants.ClientJwtDefaultLifetimeSeconds"/>. The
    /// server rejects lifetimes above
    /// <see cref="SessionConstants.ClientJwtMaxLifetimeSeconds"/>.
    /// </summary>
...
/// private key.
/// </summary>
public sealed record SessionClientAuthWithSigner : SessionClientAuth
{
    public required SessionClientAuthSigner Signer { get; init; }
}
